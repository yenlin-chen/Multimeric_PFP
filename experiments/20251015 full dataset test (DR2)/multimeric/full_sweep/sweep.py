...
root_dir = os.path.normpath(os.getcwd()+'/..' * 4)
package_dir = os.path.join(root_dir, 'src')
exp_root_dir = os.path.join(os.getcwd()+'/..' * 2)
...
sweep_config = {
    'name': f'full_sweep-{dataset_version}-multimeric-all',
    'method': 'bayes',
    'metric': {
    ...
    'parameters': {
        ...
        'edge_types_to_use': {'values': [['contact', 'codir', 'coord', 'deform']]},
        'edge_policy': {'values': ['1CONT', '0.5CONT', '1.5SIGMA', '2SIGMA', '5PAIR', '10PAIR']},
        'use_monomers': {'values': [False]},

        'node_feat_name': {'values': ['x', 'res1hot']}, # 'res1hot'
        # 'node_feat_size': {'min': , 'max': },
        'gnn_type': {'values': ['gcn']}, # 'gat', 'gin'
        'gat_atten_heads': {'values': [None]},
        'n_gnn_layers': {'min': 1, 'max': 7},
        # 'n_gnn_layers': {'values': [1, 3, 5]},
        'dim_nodes': {'min': 8, 'max': 1024},
        # 'dim_nodes': {'values': [8, 16, 32, 64, 128, 256, 512, 1024]},
        'conv_norm': {'values': [True, False]}, # True-only
        'norm_graph_input': {'values': [True, False]}, # False-only
        ...
        'n_fc_hidden_layers': {'min': 1, 'max': 5},
        # 'n_fc_hidden_layers': {'values': [2]},
        'fc_norm': {'values': [True, False]}, # True-only
        'norm_fc_input': {'values': [False]}, # False-only
        'fc_dropout_rate': {'min': 0.0, 'max': 0.5},
        # 'fc_dropout_rate': {'values': [0]},
    }
}
...
if __name__ == '__main__': # REQUIRED TO ENABLE MULTIPROCESSING
    ...
    wandb.agent(sweep_id, function=main, count=100000)
